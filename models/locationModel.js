...
async function create(location) {
  ...
  const previousLocation = await db("locations").where({ latitude, longitude }).first();
  if (previousLocation) {
    throw new Error(`Location at latitude '${ latitude }' and longitude '${ longitude }' already exists.`);
  }
  return db("locations")
    .insert(location, "id")
